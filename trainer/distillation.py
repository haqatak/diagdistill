...
class Trainer:
    ...
    def __init__(self, config):
        ...
        self.disable_wandb = self.disable_wandb or (wandb_mode == "disabled")
        ...
        if self.is_main_process:
            if SummaryWriter is not None:
                self.tb_writer = SummaryWriter(log_dir=self.output_path)
                print(f"TensorBoard logdir: {self.output_path}")
            else:
        ...
        if self.use_one_logger and OneLoggerUtils is not None and dist.get_rank() == 0 and not self.disable_wandb:
            ...
            if self.use_one_logger and self.is_main_process and OneLoggerUtils is None:
                print("Warning: one_logger is disabled (one_logger_utils module not available)")
        ...
        try:
            while True:
                ...
                if self.is_main_process:
                    ...
                    if self.tb_writer is not None:
                        self.tb_writer.add_scalar("train/per_iteration_time", iteration_time, self.step)
